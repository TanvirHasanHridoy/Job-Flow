...
- Dedicated Branch Workflow: Every time an implementation plan is approved and implementation begins, immediately create and checkout a new dedicated feature branch (e.g. `feature/bullet-matrix-cv`) before writing code.
- Granular Incremental Commits: For multi-phase implementation plans or multi-feature tasks, perform descriptive git commits immediately after completing each discrete task/phase rather than one monolithic commit at the end, enabling clean rollbacks and transparent tracking.
